...
ReactDOM.createRoot(document.getElementById('root')).render(
  <React.StrictMode>
    <App />
        ...
        if (!files['src/index.css'] && !files['src/App.css']) {
...
#root {
  ...
  min-height: 100vh;
          ...
          logger.info('Created default src/index.css');
        }

        // Create index.html - required for Vite builds
...
<html lang="en">
  ...
  <body>
    ...
    <script type="module" src="/src/main.jsx"></script>
        ...
        logger.info('Created index.html for Vite');
...
<body class="bg-gray-50 min-h-screen flex items-center justify-center p-4">
  ...
  <script>
    ...
    // Validate and fix missing required files for each framework
    if (detectedFramework === 'react' || detectedFramework === 'react-vite') {
      logger.info('Validating React project structure...');

      // Ensure required files exist
      if (!extractedFiles['vite.config.js']) {
        extractedFiles['vite.config.js'] = `import { defineConfig } from 'vite'
import react from '@vitejs/plugin-react'
...
export default defineConfig({
  ...
  build: {
    ...
    sourcemap: false
        ...
        logger.info('Added missing vite.config.js');
      }
...
<html lang="en">
  ...
  <body>
    ...
    <script type="module" src="/src/main.jsx"></script>
        ...
        logger.info('Added missing index.html');
      } else {
        // Replace LLM-generated index.html with proper Vite template
...
<html lang="en">
  ...
  <body>
    ...
    <script type="module" src="/src/main.jsx"></script>
      ...
      // Ensure main.jsx has proper imports
      if (extractedFiles['src/main.jsx'] && !extractedFiles['src/main.jsx'].includes('./index.css')) {
        const currentMain = extractedFiles['src/main.jsx'];

        if (!currentMain.includes("import './index.css'") && !currentMain.includes("import './App.css'")) {
          // Add CSS import after other imports
          const lines = currentMain.split('\n');
          const importEndIndex = lines.findIndex((line) => !line.trim().startsWith('import') && line.trim() !== '');

          if (importEndIndex > 0) {
            lines.splice(importEndIndex, 0, "import './index.css'");
            extractedFiles['src/main.jsx'] = lines.join('\n');
            logger.info('Added CSS import to main.jsx');
          }
        }
      }

      // Ensure CSS file exists
      if (!extractedFiles['src/index.css'] && !extractedFiles['src/App.css']) {
        extractedFiles['src/index.css'] = `body {
  margin: 0;
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen',
    'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue',
    sans-serif;
...
#root {
  ...
  min-height: 100vh;
        ...
        logger.info('Added default src/index.css');
      }
