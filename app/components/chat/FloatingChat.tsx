...
export function FloatingChat({ children, chatStarted }: FloatingChatProps) {
  ...
  const chat = useStore(chatStore) as { started: boolean; aborted: boolean; showChat: boolean; title?: string };
  ...
  const getBottomRightPosition = () => {
    ...
    const chatWidth = typeof size.width === 'string' ? (parseInt(size.width) / 100) * window.innerWidth : size.width;

    // Get chat height as a number
    const chatHeight =
      typeof size.height === 'string' ? (parseInt(size.height) / 100) * window.innerHeight : size.height;
  ...
  const adjustChatPosition = () => {
    if (!chatRef.current || minimized) {
      return undefined;
    }

    const newPosition = { ...position };
    const chatRect = chatRef.current?.resizableElement?.current?.getBoundingClientRect();
    ...
    if (newPosition.x + chatRect.width > window.innerWidth) {
      newPosition.x = window.innerWidth - chatRect.width;
    }

    if (newPosition.y + chatRect.height > window.innerHeight) {
      newPosition.y = window.innerHeight - chatRect.height;
    }
      ...
      try {
        ...
        if (savedSize) {
          ...
          setSize({
            width: Math.max(
              MIN_CHAT_SIZE.width,
              Math.min(
                typeof MAX_CHAT_SIZE.width === 'string'
            ...
            height: Math.max(
              ...
              Math.min(
                typeof MAX_CHAT_SIZE.height === 'string'
                  ? (parseInt(MAX_CHAT_SIZE.height) / 100) * window.innerHeight
                  : MAX_CHAT_SIZE.height,
                parsedSize.height,
              ),
            ),
          });
        ...
        if (savedPosition && savedUserPositioned === 'true') {
          ...
          setPosition({
            x: Math.min(
              parsedPosition.x,
              Math.max(20, window.innerWidth - (savedSize ? JSON.parse(savedSize).width : DEFAULT_CHAT_SIZE.width)),
            ),
            y: Math.min(
              parsedPosition.y,
              Math.max(20, window.innerHeight - (savedSize ? JSON.parse(savedSize).height : DEFAULT_CHAT_SIZE.height)),
            ),
          });
        }
    ...
    >
      ...
      >
        <div className="drag-handle flex-1 px-4 py-2.5 flex items-center justify-between">
          <span className="truncate text-sm font-medium text-gray-700 dark:text-gray-300">{chat.title || 'Chat'}</span>
          <button
  ...
  const fullChatContent = (
    <Rnd
      ...
      onDrag={(e, d) => {
        const currentWidth =
          typeof size.width === 'string' ? (parseInt(size.width) / 100) * window.innerWidth : size.width;
        const currentHeight =
          typeof size.height === 'string' ? (parseInt(size.height) / 100) * window.innerHeight : size.height;
    ...
    >
      ...
      <div className="flex flex-col h-full w-full">
        ...
        <div className="drag-handle flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 cursor-move flex-shrink-0">
          <span className="font-medium text-gray-700 dark:text-gray-300 truncate">{chat.title || 'Chat'}</span>
   ...
   */
  return createPortal(
    <TooltipProvider>{minimized ? minimizedContent : fullChatContent}</TooltipProvider>,
    document.body,
  );
}
