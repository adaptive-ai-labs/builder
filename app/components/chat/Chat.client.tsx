...
const processSampledMessages = createSampler(
  (options: {
    ...
    const { messages, isLoading, parseMessages, storeMessageHistory } = options;
...
export const ChatImpl = memo(({ description, storeMessageHistory, importChat, exportChat }: ChatProps) => {
  ...
  const [runner] = useState(
    () => new ActionRunner(Promise.resolve(null as unknown as WebContainer), () => null as unknown as BoltShell),
  );
  ...
  const { parsedMessages, parseMessages: rawParseMessages } = useMessageParser();
  const parseMessages = useCallback(
    (messages: Message[]) => {
      rawParseMessages(messages, isLoading);
    },
    [rawParseMessages, isLoading],
  );
  ...
  const runAnimation = async () => {
    ...
    await animate('div', { y: [0, -16, 0], opacity: [1, 0.5, 1] }, { duration: 0.5, ease: cubicEasingFn });
      ...
      if (!chatStarted) {
        ...
        if (autoSelectTemplate) {
          try {
            ...
            const temResp = await selectStarterTemplate({ message: value, model, provider });

            if (temResp) {
              const { template, title } = temResp;
              setMessages([
                {
                  id: `1-${new Date().getTime()}`,
                  role: 'user',
                  content: `Selected template: ${template}\nTitle: ${title}`,
                },
                {
                  id: `2-${new Date().getTime()}`,
                  role: 'assistant',
                  content: `I'll help you set up a project using the ${template} template with title: ${title}`,
                  annotations: ['hidden'],
                },
              ]);
              reload();
